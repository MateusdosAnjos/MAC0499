...
func string_comparision_routine(input, arguments, action_number):
...
func number_comparision_routine(input, arguments, number_type, action_number):
...
func execute(input, arguments, player_answer, action_number):
    ...
    if (key_words != null):
        if (typeof(input) == TYPE_STRING):
            return string_comparision_routine(input, key_words, action_number)
        elif (typeof(input) == TYPE_INT):
           return number_comparision_routine(input, key_words, TYPE_INT, action_number)
        elif (typeof(input) == TYPE_REAL):
           return number_comparision_routine(input, key_words, TYPE_REAL, action_number)
    else:
        $ErrorMessages.show_error_message("if/else", action_number)
        return [input, false]

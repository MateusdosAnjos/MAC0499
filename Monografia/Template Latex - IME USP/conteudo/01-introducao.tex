...
Para entender melhor o que é uma Árvore de Cena deve-se entender um pouco sobre
o modo como Godot trabalha internamente. A única instância que é executada no 
...
internamente (você pode verifiar o arquivo "main.cpp" no código-fonte se 
estiver interessado em ver como isso funciona internamente).

O programa do usuário, ou jogo, inicia no \textit{MainLoop}. Essa classe possui
alguns métodos, para inicialização, ocioso (retorno de chamada sincronizado por 
quadro), fixo (retorno de chamada sincronizado com física) e entrada. Novamente,
esse é um nível baixo e, ao fazer jogos em Godot, escrever seu próprio MainLoop

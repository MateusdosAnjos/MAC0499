...
Depois que o sistema está totalmente conectado, podemos ter o seguinte exemplo
de uma árvore de execução:

\begin{figure}[H]
    \includegraphics[width=\textwidth]{../figuras/arvore_execucao_redim.png}
    \caption{Exemplo de Árvore de Execução}
\end{figure}

Lembre-se que os filhos da esquerda, conhecidos como \textit{left\_child}, que 
não foram representados na figura, para melhorar a apresentação edeixá-la mais 
fácil de entender, existem e possuem o valor \textit{null}.
...
\begin{itemize}
    \item[$\bullet$]
        \textit{starting\_pos} e \textit{finish\_pos} - demarcam onde o 
        processo visual inicia e onde ele termina, respectivamente.
    \item[$\bullet$]
        \textit{start\_input} - recebe o valor inicial da entrada.
    \item[$\bullet$]
        \textit{curve} - guarda a curva 
        de pontos, que será preenchida conforme a execução do programa.
    \item[$\bullet$] 
        \textit{total\_inputs} - guarda a quantidade de valores que foi 
        fornecido na entrada do programa
    \item[$\bullet$]
        \textit{current\_start\_input} - guarda qual valor da entrada que será
        processado.
    \item[$\bullet$]
        \textit{processed\_input} - armazena os 
        valores que se alteram durante a execução e que são exibidos na 
        animação.
    \item[$\bullet$] 
        \textit{CurrentActionNode} - guarda qual espaço de ação que será
        executado
    \item[$\bullet$] 
        \textit{is\_exit\_sucess} - sinaliza se houve algum erro 
        durante a execução.
\end{itemize}
...
então será seguido o caminho do filho da esquerda.
